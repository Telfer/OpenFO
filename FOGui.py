...
class FOBaseWidget(QtGui.QWidget):
...
class FODockWidget(FOBaseWidget):
...
class FOLineEdit(QtGui.QLineEdit):
...
class FOTaskPanel:
    ...
    def accept(self):
        if hasattr(FreeCADGui,"FOToolBar"):
            return FreeCADGui.FOToolBar.validatePoint()
        else:
            if FreeCADGui.ActiveDocument:
                FreeCADGui.ActiveDocument.resetEdit()
            return True
    def reject(self):
        FreeCADGui.FOToolBar.isTaskOn = False
        FreeCADGui.FOToolBar.escape()
        if FreeCADGui.ActiveDocument:
            FreeCADGui.ActiveDocument.resetEdit()
        return True
...
class FOToolBar:
    ...
    def __init__(self):
        ...
        self.linewidth = 2#Draft.getParam("linewidth",2)
        self.fontsize = 0.20#Draft.getParam("textheight",0.20)
...
#---------------------------------------------------------------------------
    ...
    def _lineedit (self,name, layout, hide=True, width=None):
        p = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/General")
        bsize = p.GetInt("ToolbarIconSize",24)-2
        lineedit = FOLineEdit(self.baseWidget)
...
#---------------------------------------------------------------------------
    ...
    def taskUi(self,title="Draft",extra=None,icon="Draft_Draft"):
        ...
        self.panel = FOTaskPanel(self.baseWidget,extra)
...
#---------------------------------------------------------------------------
    ...
    def displayPoint(self, point=None, last=None, plane=None, mask=None):
        ...
        if last and dp and plane:
            length, theta, phi = get_spherical_coords(*dp)
    ...
    def update_spherical_coords(self):
        length, theta, phi = get_spherical_coords(self.x,self.y,self.z)
        self.lvalue = length
        self.pvalue = math.degrees(theta)
        self.avalue = math.degrees(phi)
        self.angle = FreeCAD.Vector(get_cartesian_coords(
            1, theta, phi))
    ...
    def update_cartesian_coords(self):
        self.x, self.y, self.z = get_cartesian_coords(
...
if not hasattr(FreeCADGui,"FOToolBar"):
    FreeCADGui.FOToolBar = FOToolBar()